...
context("Actions", () => {
    ...
    it("Check ECommerce Payment", () => {
        ...
        cy.get('[data-testid="shopping-cart-btn"]').last().click();
        ...
        cy.get(".MuiTableCell-body").last().should("have.text", "$4");
        cy.get(".MuiTableCell-alignRight").contains("$4");
        cy.get('img[alt="stripe"]').click();
        cy.contains("Payment");
    });
});
